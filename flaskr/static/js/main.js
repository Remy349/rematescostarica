document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ACCORDION CURSO */
  const cursosMoreContent = document.getElementsByClassName(
      'cursos__more-content'
    ),
  ...
  if (registroForm) {
    registroForm.addEventListener('submit', registroFormValidate)
  }

  /* LINK DE DESCARGA PARA ARCHIVO EXCEL */
  const downloadBtn = document.getElementById('downloadBtn')
  const downloadBtnTwo = document.getElementById('downloadBtnTwo')
  ...
  if (downloadBtnTwo) {
    downloadBtnTwo.addEventListener('click', () => {
      fetch('/create_file_two', {
        ...
        .then((resp) => resp.json())
        .then((data) => {
          const downloadLinkTwo = document.getElementById('downloadLinkTwo')

          downloadLinkTwo.style.display = 'block'
          downloadLinkTwo.innerHTML = `
                        <a href="${location.origin}/static/files/Registros.xlsx" class="download-link" download>
                            Descargar archivo - Registros.xlsx
                        </a>
                    `

          setTimeout(() => {
            downloadLinkTwo.style.display = 'none'
          }, 30000)
        })
        .catch((error) => console.log(error))
    })
...
const editarUserValidate = (e) => {
  ...
  if (notWhiteSpace.test(firstname)) {
    ...
    errorInput(
      firstnameCont,
      firstnameLabel,
      'Nombre no puede ser menor a 3 caracteres!'
    )
    errors = true
  ...
  if (notWhiteSpace.test(lastname)) {
    ...
    errorInput(
      lastnameCont,
      lastnameLabel,
      'Apellido no puede ser menor a 4 caracteres!'
    )
    errors = true
    ...
    errorInput(
      lastnameCont,
      lastnameLabel,
      'Apellido no puede ser mayor a 15 caracteres!'
    )
    errors = true
  ...
  if (phone.length == 0) {
    ...
    errorInput(
      phoneCont,
      phoneLabel,
      'Telefono no puder ser menor a 8 digitos!'
    )
    errors = true
    ...
    errorInput(
      phoneCont,
      phoneLabel,
      'Telefono no puede ser mayor a 18 digitos!'
    )
    errors = true
    ...
    errorInput(
      phoneCont,
      phoneLabel,
      'Telefono solo debe contener digitos entre [0-9]!'
    )
    errors = true
  } else {
  ...
  if (notWhiteSpace.test(email)) {
    ...
    errorInput(
      emailCont,
      emailLabel,
      'Dirección E-Mail no permitida! Usa el siguiente formato "ejemplo@ejemplo.com"'
    )
    errors = true
    ...
    errorInput(
      emailCont,
      emailLabel,
      'Dirección E-Mail muy corta. No menos de 10 caracteres!'
    )
    errors = true
    ...
    errorInput(
      emailCont,
      emailLabel,
      'Dirección E-Mail muy larga. No mas de 150 caracteres!'
    )
    errors = true
  } else {
...
const validarFormulario = (e) => {
  ...
  if (notWhiteSpace.test(username)) {
    ...
    errorInput(
      usernameCont,
      usernameLabel,
      'Nombre de usuario no puede ser menor a 5 caracteres!'
    )
    errors = true
    ...
    errorInput(
      usernameCont,
      usernameLabel,
      'Nombre de usuario no puede ser mayor a 15 caracteres!'
    )
    errors = true
  } else {
  ...
  if (notWhiteSpace.test(password)) {
    ...
    errorInput(
      passwordCont,
      passwordLabel,
      'Contraseña no puede ser menor a 8 caracteres!'
    )
    errors = true
  } else {
  ...
  if (notWhiteSpace.test(passwordT)) {
    ...
    errorInput(
      passwordContT,
      passwordLabelT,
      'Confirmar contraseña no puede estar vacia!'
    )
    errors = true
